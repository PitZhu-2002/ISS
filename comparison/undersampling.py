...
from imblearn.under_sampling import RandomUnderSampler, TomekLinks, ClusterCentroids, OneSidedSelection, NearMiss, \
...
def start(name,data,times,base_copy,base_name,timeframe):
    ...
    for train_index, test_index in stf_split.split(X, y):
        ...
        samplers = {
            ...
            'CNN': CondensedNearestNeighbour(random_state = 10000),
            'OSS': OneSidedSelection(random_state = 10000),
            'CC': ClusterCentroids(random_state = 10000),
            'TomekLink': TomekLinks(),
            'NM': NearMiss(),
            'SMOTEENN': SMOTEENN(random_state = 10000)
        }
...
if __name__ == '__main__':
    ...
    timeframe = '3'
    names = ['RUS','CNN','OSS','CC','TomekLink','NM','SMOTEENN']
