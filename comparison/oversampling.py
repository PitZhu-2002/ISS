...
from smote_variants import MWMOTE, SOMO, ADASYN, SYMPROD, Safe_Level_SMOTE, Borderline_SMOTE1, kmeans_SMOTE, ANS, NRAS
...
def start(name,data,times,base_copy,base_name,timeframe):
    ...
    for train_index, test_index in stf_split.split(X, y):
        ...
        samplers = {
            ...
            'MWMOTE': MWMOTE(random_state = 10000),
            'SOMO': SOMO(random_state = 10000),
            'ADASYN': ADASYN(random_state = 10000),
            'SYMPROD': SYMPROD(random_state = 10000),
            'SLS': Safe_Level_SMOTE(random_state = 10000),
            'Borderline_SMOTE': Borderline_SMOTE1(random_state = 10000),
            'kmeans_SMOTE': kmeans_SMOTE(random_state = 10000),
            'ANS': ANS(random_state = 10000),
            'NRAS': NRAS(random_state = 10000)
        }
        for m in range(len(name)):
            base = deepcopy(base_copy)
            n = name[m]
            X_sam, y_sam = samplers[n].fit_resample(X_prime,y_prime)
...
if __name__ == '__main__':
    ...
    # Key Parameter that controls the large-scale experiment
    timeframe = '3'
    names = ['ROS','SMOTE','MWMOTE','SOMO','ADASYN','SYMPROD','SLS','Borderline_SMOTE','kmeans_SMOTE','ANS','NRAS']
